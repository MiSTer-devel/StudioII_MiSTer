...
The Studio II beeper targets E♭4 (311.127 Hz via a fractional divider) and follows a delayed, rounded pitch droop to C4 over about 0.4 seconds. The pitch-control state recovers progressively while Q is low, so a closely following beep can expose a short upward settling glide. Q itself remains a simple on/off gate; this is not an ADSR note envelope. Amplitude shaping still needs final tuning against real-hardware recordings.
