...
- Never run Quartus synthesis. Default to the local Quartus 17.0.x GUI when a build is needed, and review the reports the user provides. Suggest scripted builds only when requested, using the verified local installation; do not default to the Docker-only `tools/quartus-build.sh`. See `docs/development.md` for local setup.
